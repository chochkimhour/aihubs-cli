...
} from "../dist/grok/usage-provider.js";
...
test("supports missing usage and missing reset without inventing values", () => {
  ...
  assert.equal(
    normalizeGrokBilling({ config: { creditUsagePercent: 18 } }).resetAt,
    undefined,
  );
});
...
test("classifies auth, server, and malformed responses safely", async () => {
  for (const [status, kind] of [
    ...
    await assert.rejects(
      ...
      (error) => error instanceof GrokUsageError && error.kind === kind,
    );
  }
  const provider = new GrokUsageProvider({
  ...
  await assert.rejects(
    () => provider.get("01", { key: "fake", user_id: "u" }),
    (error) => error instanceof GrokUsageError && error.kind === "unsupported",
  );
});
